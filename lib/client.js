...
`;
    ...
    function SubagentProgressDock({ sessionId, useSessions, openChild, controlChild, refreshCatalog, t }) {
      ...
      React.useEffect(() => {
        if (catalog === undefined) void refreshCatalog?.();
      }, [catalog]);
      ...
      const actOnChild = async (childId, action) => {
        ...
        try {
          const ok = await controlChild(sessionId, childId, action, t('action.retryPrompt'));
    ...
    function apply(ctx) {
      ...
      const controlChild = async (parentSessionId, childId, action, promptText) => {
        try {
          ...
          if (result?.ok !== true) {
            console.error('[subagent-progress] subagents.interruptByParent rejected', { parentSessionId, childId, result });
            return false;
          }
          return true;
        } catch (error) {
        ...
        ctx.slots.register(
          {
            ...
            inject: (sessionId) => ({
              controlChild,
              refreshCatalog: () => sessions.refreshSubagents?.(sessionId),
