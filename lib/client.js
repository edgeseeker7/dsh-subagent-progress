...
`;
    ...
    function Chip({
      ...
      // next = pending selection). Provider and effort ride the tooltip.
      const modelSel = summary.projectionValues?.modelSelection;
      const modelName = modelSel?.lastUsed?.model ?? modelSel?.next?.model ?? null;
      const modelTitle =
        modelName === null
          ? undefined
          : `${modelSel?.lastUsed?.provider ?? modelSel?.next?.provider ?? ''}/${modelName}${
              (modelSel?.lastUsed?.reasoningEffort ?? modelSel?.next?.reasoningEffort)
                ? ` · effort ${modelSel?.lastUsed?.reasoningEffort ?? modelSel?.next?.reasoningEffort}`
                : ''
            }`;
      const modelChip =
        modelName === null ? null : h('span', { className: 'dsh-sp-model', title: modelTitle }, modelName);
