...
/* Model tag: which model this subagent is actually running (from the host's
   official modelSelection projection — no extra traffic). Quiet chip, same
   weight class as the disclosure affordance. */
.dsh-sp-model {
  flex: none; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  padding: 0 6px; border-radius: 6px;
  font-size: 11px; font-variant-numeric: tabular-nums; line-height: 16px;
  color: var(--dsw-alias-label-tertiary);
  background: color-mix(in srgb, var(--dsw-alias-label-tertiary, #878e9d) 10%, transparent);
}
@media (prefers-reduced-motion: reduce) {
