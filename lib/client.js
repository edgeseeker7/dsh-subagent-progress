...
.dsh-sp-dock {
  ...
  /* Wider than the composer card (which is chat-width + 32px): the dock is an
     information panel, so it may use more of the conversation column. */
  max-width: min(1080px, calc(100% - var(--dsh-composer-side-clearance) - var(--dsh-composer-side-clearance)));
  margin: 0 auto;
...
`;
    ...
    function formatAction(progress) {
      ...
      if (action.name === 'bash' || action.name === 'pwsh') return action.arg ? `${action.name}: ${action.arg}` : action.name;
    ...
    function Chip({ summary, now, openChild, t }) {
      ...
      // transcript text; strip markdown markers to keep one clean line.
      const rawPreview = progress?.lastUpdate?.message ?? todos?.current ?? progress?.lastText;
      ...
      const statusText = [live ? t('chip.running') : t('chip.idle'), elapsed !== null ? formatDuration(elapsed, t) : null]
        .filter(Boolean)
        .join(' · ');
