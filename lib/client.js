...
`;
    ...
    function Chip({ summary, now, openChild, t, update }) {
      ...
      // Preview priority: current todo item > raw transcript text. The
      // update lives in its own section at the bottom of this same card, so
      // the preview never competes with it; lastText in that situation is
      // just the long raw version of the same report.
      const rawPreview = update
        ? (todos?.current ?? null)
        : (todos?.current ?? progress?.lastUpdate?.message ?? progress?.lastText);
      const preview =
        rawPreview
          ?.replace(/[*`#]+/g, '')
          .replace(/\s+/g, ' ')
