...
`;
    ...
    function Chip({ summary, now, openChild, t, updateShown }) {
      ...
      // row below, the card yields to avoid showing the same text twice.
      const rawPreview = (updateShown ? null : progress?.lastUpdate?.message) ?? todos?.current ?? progress?.lastText;
      const preview =
        rawPreview
          ?.replace(/[*`#]+/g, '')
          .replace(/\s+/g, ' ')
