...
.dsh-sp-preview {
  ...
  /* Reserved even when empty: uniform card height across the grid. */
  min-height: 17px;
...
.dsh-sp-card-update {
  ...
  /* Reserved even without an update: uniform card height across the grid. */
  min-height: 24px;
}
...
`;
    ...
    function Chip({ summary, now, openChild, t, update }) {
      ...
      // Uniform four-row card structure — every card renders the same rows
      // in the same order so any number of cards aligns perfectly in the
      // grid: head / meta / current-task / update. Missing content reserves
      // its line (blank task row, muted fallback in the update row).
      ...
      return h(
        ...
        h('span', { className: 'dsh-sp-preview' }, preview ?? ''),
        update !== null && update !== undefined
          ? h(
              'span',
              {
                ...
                // One line on screen; the full message is a hover away.
          ...
          : h(
              'span',
              {
                ...
                ...(fallbackText !== null ? { title: progress?.lastText } : {}),
              },
              h('span', { className: 'dsh-sp-card-update-message' }, fallbackText ?? t('card.noUpdate')),
            ),
      );
    }
    ...
    function SubagentProgressDock({ sessionId, useSessions, openChild, t }) {
      ...
      // Standalone rows remain only for orphaned updates — findings whose
      // child no longer has a card (stopped beyond the grace window).
      ...
      const displayedIds = new Set(displayed.map((summary) => summary.id));
      const orphanUpdates = [];
      for (const summary of children) {
        const update = updateByChild.get(summary.id);
        if (update && !displayedIds.has(summary.id)) orphanUpdates.push({ summary, update });
      }
      orphanUpdates.sort((a, b) => b.update.at - a.update.at);
      const shownOrphans = orphanUpdates.slice(0, MAX_UPDATES);

      if (displayed.length === 0 && shownOrphans.length === 0) return null;
        ...
        shownOrphans.map(({ summary, update }) =>
          h(
            ...
            h(
              ...
              h('span', { className: 'dsh-sp-update-message' }, h(UpdateBody, { update, t })),
            ),
          ),
        ),
      );
