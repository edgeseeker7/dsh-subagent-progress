...
/* Model tag: which model this subagent is actually running (from the host's
   official modelSelection projection — no extra traffic). Quiet but
   discoverable: secondary label on a faint chip. */
.dsh-sp-model {
  /* Shrinkable (flex 0 1 auto): on narrow cards the chip yields before the
     head's buttons do — a rigid chip used to push × and Retry past the
     card's overflow-hidden right edge, "randomly" hiding them. */
  flex: 0 1 auto; min-width: 0; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  padding: 0 6px; border-radius: 6px;
  font-size: 11px; font-variant-numeric: tabular-nums; line-height: 16px;
  color: var(--dsw-alias-label-secondary);
  background: color-mix(in srgb, var(--dsw-alias-label-tertiary, #878e9d) 14%, transparent);
}
/* Cards enter symmetrically to how they leave: a short rise + fade. The
   dock-level dsh-sp-enter only covers the container; per-card mounts used
   to pop in abruptly. Ghosts inside .dsh-sp-leaving must not replay it. */
.dsh-sp-card { animation: dsh-sp-card-enter .22s ease-out; }
@keyframes dsh-sp-card-enter {
  from { opacity: 0; transform: translateY(6px); }
  to { opacity: 1; transform: translateY(0); }
}
.dsh-sp-leaving .dsh-sp-card { animation: none; }
@media (prefers-reduced-motion: reduce) {
  .dsh-sp-disclosure-chevron { transition: none; }
  .dsh-sp-card { animation: none; }
}
...
.dsh-sp-status {
  /* Also shrinkable with ellipsis, for the same reason as the model chip:
     buttons keep their room at any card width. */
...
`;
    ...
    /**
     * Stable spawn order, assigned on first sight. Position encodes
     * identity, not freshness: sorting live cards by `updatedAt` made the
     * whole grid reshuffle on every progress tick when several subagents
     * reported within the same poll window — the cards visibly flickered.
     * Motion is reserved for state transitions (enter/exit/fail); activity
     * updates render in place.
     */
    let spawnSeq = 0;
    const spawnOrderById = new Map();
    function spawnOrder(id) {
      let order = spawnOrderById.get(id);
      if (order === undefined) {
        order = spawnSeq++;
        spawnOrderById.set(id, order);
      }
      return order;
    }

    /** Card ordering within one tree level: live first, then failures, then stable spawn order. */
    function compareCards(a, b) {
      ...
      return spawnOrder(a.id) - spawnOrder(b.id);
    }
    ...
    function Chip({
      ...
      // Which model this child is actually running, from the host's official
      // modelSelection projection (lastUsed = the model of its real requests;
      // next = pending selection). Hover opens OUR popover with the full
      // provider/model/effort detail — a native title tooltip inside a
      // button that carries its own title is unreliable (and slow).
      const modelSel = summary.projectionValues?.modelSelection;
      const modelName = modelSel?.lastUsed?.model ?? modelSel?.next?.model ?? null;
      const modelProvider = modelSel?.lastUsed?.provider ?? modelSel?.next?.provider ?? null;
      const modelEffort = modelSel?.lastUsed?.reasoningEffort ?? modelSel?.next?.reasoningEffort ?? null;
      const modelTitle =
        modelName === null
          ? undefined
          : `${modelProvider ?? ''}/${modelName}${modelEffort !== null ? ` · effort ${modelEffort}` : ''}`;
      const modelDetail =
        modelName === null
          ? null
          : [
              `**${modelName}**`,
              ``,
              modelProvider !== null ? `- provider: \`${modelProvider}\`` : null,
              modelEffort !== null ? `- effort: \`${modelEffort}\`` : null,
            ]
      ...
      const modelChip =
        modelName === null
          ...
          : h(
              'span',
              {
                className: 'dsh-sp-model',
                title: modelTitle,
                onMouseEnter: (event) =>
                  onUpdateHover({
                    summary,
                    update: { kind: null, message: modelDetail, at: Date.now() },
    ...
    function useLeaving(entries, keyOf) {
      ...
      React.useEffect(() => {
        ...
        const purgeAt = Date.now() + EXIT_MS + 80;
        setDeparted((current) => [...current, ...leaving.map((item) => ({ ...item, purgeAt }))]);
        return undefined;
      }, [signature]);
      // Purge expired ghosts by timestamp, not per-batch timers: a batch
      // departing while another was mid-exit used to have its purge timer
      // cleared by the effect cleanup — stranding those ghosts on screen
      // forever.
      React.useEffect(() => {
        if (departed.length === 0) return undefined;
        const wait = Math.max(0, Math.min(...departed.map((item) => item.purgeAt)) - Date.now());
        const timer = setTimeout(() => {
          setDeparted((current) => current.filter((item) => item.purgeAt > Date.now()));
        }, wait + 10);
    ...
    function SubagentProgressDock({ sessionId, useSessions, openChild, controlChild, refreshCatalog, t }) {
      ...
      // HOOK-ORDER CRITICAL: every hook must run before the early return —
      // with useState below the return, an empty→non-empty transition called
      // one hook more than the previous render, React threw, and the slot's
      // error boundary left the dock permanently dead until page reload.
        ...
        hoverInfo !== null && typeof window !== 'undefined'
          ? h(
              ...
              h(
                ...
                (() => {
                  const sel = hoverInfo.summary.projectionValues?.modelSelection;
                  const name = sel?.lastUsed?.model ?? sel?.next?.model ?? null;
                  return name === null ? null : h('span', { className: 'dsh-sp-model' }, name);
                })(),
              ),
