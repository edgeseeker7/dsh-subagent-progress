...
const stateSchema = z.object({
  ...
  /**
   * Sticky latest FINDING-kind update. The harness requires every child to
   * send_message a final report, which would otherwise overwrite a mid-run
   * finding in the single lastUpdate slot — findings outlive that churn.
   */
  lastFinding: updateSchema.nullable(),
...
export const subagentProgressProjection = {
        ...
        if (COMM_TOOLS.has(toolName)) {
          ...
          if (update !== null) {
            return {
              ...
              lastTool: toolName,
              lastUpdate: update,
              // A finding is sticky: later progress/report churn must not
              // evict it (the mandatory final send_message would otherwise
              // always clobber a mid-run finding).
              ...(update.kind === 'finding' ? { lastFinding: update } : {}),
  ...
  // v7: lastFinding — sticky finding slot, immune to the mandatory final
  // send_message report overwriting lastUpdate; v6 added failure; v5
  // preserved update newlines (normalizeMarkdown).
  stateVersion: 7,
};
