...
3. **Client 半**:在 `conversation.input.dock` slot(输入框卡片正上方、整宽)渲染一排进度 chip——每个直接子 agent 一个:运行状态呼吸点、标签、`轮 N·步 M`、最近工具、耗时、最新汇报/文本预览。最新一条 `notify_user` 汇报单独成行显示在 chip 条下方(📈 进展 / ⏳ 预期 / 💡 发现)。点击 chip 经 catalog 地址(`openSubagent`)打开对应子 agent 会话。
...
复现:`dsh --profile headless --patch verify.patch.yml "<委派任务>"`(`verify.patch.yml` 把 an internal gateway provider 内联进 llm-pi-ai 配置,绕开 headless 首请求先于 settings 加载完成的 NO_ADAPTER 竞态)。
