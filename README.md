...
3. **Client 半**:在会话头部的 `utilities` slot 渲染一排进度 chip——每个直接子 agent 一个:运行状态呼吸点、标签、`轮 N·步 M`、最近工具、耗时、最新汇报/文本预览。最新一条 `notify_user` 汇报单独成行显示在 chip 条下方(📈 进展 / ⏳ 预期 / 💡 发现)。点击 chip 打开对应子 agent 会话。
