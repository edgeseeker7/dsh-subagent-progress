...
3. **Client 半**:在 `conversation.input.dock` slot 渲染**毛玻璃(glassmorphism)进度坞**,几何尺寸直接复用官方 dock 的 `--dsh-composer-card-max-width` 等变量,与输入框卡片精确对齐(不铺满全屏)——每个直接子 agent 一枚玻璃胶囊(整排居中):呼吸状态点、标签、`轮 N·步 M`、最近工具、耗时(等宽数字)、清理过 markdown 符号的单行预览;下方一条整宽汇报栏展示最新一条 `notify_user` 汇报,带彩色分类徽标(↗ 进展 / ◷ 预期 / ◆ 发现),消息体经宿主 `MarkdownText` 组件渲染(支持 **粗体**、`code`、列表等)。响应式:窄屏下胶囊换行居中、预览隐藏、汇报栏消息自动折行。全部颜色取自宿主 `--dsw-*` 主题变量(亮/暗自适应),样式表按官方约定在模块物化期注入并打 `data-plugin-css` 标(HMR 可认领)。点击胶囊/汇报栏经 catalog 地址(`openSubagent`)打开对应子 agent 会话。
